...
import { trackContactAction, trackEvent } from '@/lib/analytics';
...
export default function HeroSection() {
  ...
  const scrollToContact = () => {
    trackContactAction('ClickHeroCTA', 'hero_primary');
    const el = document.querySelector('#contact');
    if (el) el.scrollIntoView({ behavior: 'smooth' });
  };

  const scrollToPortfolio = () => {
    trackEvent('ClickPortfolioCTA', { location: 'hero_secondary' });
    ...
    >
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 relative z-10 text-center mt-0 md:mt-48">
        ...
        <div className="badge-gradient mb-6 mx-auto inline-flex">
          <span className="w-2 h-2 rounded-full bg-primary animate-pulse-glow" />⭐ 120+ dự án ·
          5.0/5 đánh giá · Bàn giao 5 ngày
        </div>
        ...
        >
          Website đẹp xịn, <span className="highlight-orange">lên sóng</span> trong{' '}
        ...
        >
          Khách hàng{' '}
          <span className="highlight-word font-semibold text-foreground">tìm bạn trên Google</span>{' '}
          thay vì chỉ Facebook — website chuyên nghiệp, chuẩn SEO, tích hợp{' '}
          <span className="font-semibold text-[#0068FF]">Zalo</span> chat. Giao đúng 5 ngày,{' '}
          <span className="highlight-word font-semibold text-foreground">bảo hành 6 tháng</span>.
        </p>
